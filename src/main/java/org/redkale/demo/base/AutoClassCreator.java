...
public class AutoClassCreator {
    ...
    private static String createEntityContent(String pkg, String classname, String superclassname) throws Exception {
        ...
        source.executeQuery("SHOW CREATE TABLE " + classname.toLowerCase(), (DataResultSet tcs) -> {
            try {
                tcs.next();
        ...
        if (superclassname != null && !superclassname.isEmpty()) {
            source.executeQuery("SELECT * FROM information_schema.columns WHERE  table_name = '" + superclassname.toLowerCase() + "'", (DataResultSet rs) -> {
                try {
                    while (rs.next()) {
        ...
        source.executeQuery("SELECT * FROM information_schema.columns WHERE  table_name = '" + classname.toLowerCase() + "'", (DataResultSet rs) -> {
            try {
